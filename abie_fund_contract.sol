...
contract Ballot {
    ...
    function addTokenToBalance(uint j){
      members[msg.sender].balnce+=j;
    }

    modifier Allowed(){
        if (members[msg.sender].numberOfVotes>0 && current_block_number<deadline && members[msg.sender].balance>0 && members[msg.sender]!=proposals[i].creator) throw;
        _;
    }

    function Vote(uint prop) Allowed(){
